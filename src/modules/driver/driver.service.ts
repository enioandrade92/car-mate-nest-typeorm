...
export class DriverService {
	...
	async createDriver(createDriver: CreateDriverDto) {
		try {
			const existDriver = await this.repository.findByName(createDriver.name);
			if (existDriver) {
			...
			this.logger.log(`Created successfully the driver id ${createdDriver.id}`);
	...
	async updateDriver(id: number, updateDriver: UpdateDriverDto) {
		try {
			...
			const updatedDriver = await this.repository.update(driver, updateDriver);
	...
	async findDriverByFilters({ name, page = 1, limit = 10 }: FilterDriverDto) {
		try {
			...
			this.logger.error(`Failed to find the driver. Error: ${error.message}.`);
			throw new HttpException(
				error.message,
				error.status || HttpStatus.INTERNAL_SERVER_ERROR,
			);
